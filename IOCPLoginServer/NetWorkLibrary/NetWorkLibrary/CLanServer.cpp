...
bool CLanServer::Disconnect(uint64_t sessionID)
{
    ...
    if (m_sessions[index]->sessionID != sessionID)
    {
        return true;
    }
...
unsigned __stdcall CLanServer::WorkerThread(LPVOID param)
{
    ...
    if (g_dbInited == false)
    {
        if (g_db.Connect("127.0.0.1", "root", "game123", "accountdb", 3306))
        {
            g_dbInited = true;
            InterlockedIncrement(&totalDBConnect);
        }
    }
    ...
    if (g_dbInited)
    {
		g_db.Disconnect();
        g_dbInited = false;
		InterlockedDecrement(&totalDBConnect);
    }
