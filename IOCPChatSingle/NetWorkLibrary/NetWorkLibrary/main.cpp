...
int main()
{
    ...
    {
        printf("Server Setting Parser Clear!\n");
    }
    else
    {
        printf("Server Setting Parser something wrong\n");
        return 1;
    }
...
unsigned __stdcall MonitoringThreadProc(void* p)
{
    ...
    mpMonitorLogin(packet, Chatserver);
    ...
    while (true)
    {
        ...
        if (now >= nextPrint)
        {
            ...
            //int stackSize = server->m_freeIndices.size();
            ...
            printf("CharacterCnt : %lu / SectorCnt : %d\n", server.currentPlayerCnt, CheckTotalSector());
            printf("packetbufferCnt : %lu\n", TLSMemoryPool<PacketBuffer>::GetCount());
            printf("jobPacketCnt : %lu\n", TLSMemoryPool<JobMessage>::GetCount());
            printf("packetReadBufferCnt : %lu\n", TLSMemoryPool<PacketBufferReader>::GetCount());
            printf("acceptTPS : %lu | jobQueueTPS : %lu | MoveStartTps : %lu | MoveStopTps : %lu | ChatStartTps : %lu | ChatEndTps : %lu | HeartBeatTps : %lu\n"
                , server.acceptTPS, jobQueueTPS, MoveStartTps, MoveStopTps, ChatStartTps, ChatEndTps, heartBeatTps);
            printf("jobqueueSize : %d\n", jobQueue.Size());
            printf("sendFrame : %lu\n", sendCnt);
            printf("sendCheck : %lu\n", sendCheckTest);
            sendCnt = 0;

            printf("Disconnect  protocol : %lu | checksum : %lu\n", server.disconnectCntProtocol, server.disconnectCntCheckSum);
            /////PDHMonitoring and CPUusage
            printf("------------PDH--------------\n");

            PacketBuffer* buffer0 = client.CreatePacketBuffer();
            mpMonitorDataUpdate(buffer0, dfMONITOR_DATA_TYPE_CHAT_SERVER_RUN, isClientConnected, now);
            ...
            mpMonitorDataUpdate(buffer1, dfMONITOR_DATA_TYPE_CHAT_SERVER_CPU, ProcessTime.GetTotal(), now);
            ...
            mpMonitorDataUpdate(buffer2, dfMONITOR_DATA_TYPE_CHAT_SERVER_MEM, pdhMonitor.GetProcessUserMemory(), now);
            ...
            mpMonitorDataUpdate(buffer3, dfMONITOR_DATA_TYPE_CHAT_SESSION, cnt, now);
            Lan_SendPacket_Unicast(clientID, buffer3);
            buffer3->Release();

            PacketBuffer* buffer4 = client.CreatePacketBuffer();
            mpMonitorDataUpdate(buffer4, dfMONITOR_DATA_TYPE_CHAT_PLAYER, server.currentPlayerCnt, now);
            Lan_SendPacket_Unicast(clientID, buffer4);
            buffer4->Release();

            PacketBuffer* buffer5 = client.CreatePacketBuffer();
            mpMonitorDataUpdate(buffer5, dfMONITOR_DATA_TYPE_CHAT_UPDATE_TPS, jobQueueTPS, now);
            Lan_SendPacket_Unicast(clientID, buffer5);
            buffer5->Release();

            PacketBuffer* buffer6 = client.CreatePacketBuffer();
            mpMonitorDataUpdate(buffer6, dfMONITOR_DATA_TYPE_CHAT_PACKET_POOL, TLSMemoryPool<PacketBuffer>::GetCount(), now);
            Lan_SendPacket_Unicast(clientID, buffer6);
            buffer6->Release();

            PacketBuffer* buffer7 = client.CreatePacketBuffer();
            mpMonitorDataUpdate(buffer7, dfMONITOR_DATA_TYPE_CHAT_UPDATEMSG_POOL, TLSMemoryPool<JobMessage>::GetCount(), now);
            Lan_SendPacket_Unicast(clientID, buffer7);
            buffer7->Release();
