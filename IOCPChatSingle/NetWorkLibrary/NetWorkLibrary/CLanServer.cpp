...
{ 
    ...
    while (cnt <= 1)
    {
        ...
        if (bufCount > 400) // 최대 버퍼 개수 제한 (예: 100개로 설정, 필요에 따라 조정 가능)
            bufCount = 400;
    ...
    while (true)
    {
      ...
      /*  if (session == nullptr && overlapped == nullptr)
        ...
        else if (overlapped == &session->sendOverlapped)
        {
            ...
            //sendCnt--
            InterlockedIncrement(&sendCheckTest);
