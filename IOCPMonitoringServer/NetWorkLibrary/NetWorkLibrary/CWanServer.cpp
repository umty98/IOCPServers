...
void CWanServer::ReleaseSession(SESSION* session)
{
   ...
   // printf("release remainCnt : %d\n", remainCnt);
    ...
    //연결을 끊으면 new로 만든 packetbuffer가 delete되지 않아서 여기서 하는 방법을 생각해봄
    //int remainCnt = session->sendQueue.Size();

    //세션 delete안하고 재사용
    //delete session;
}

int CWanServer::GetSessionCount()
{
    int cnt = 0;
    // m_sessions는 고정 크기이므로 락 없이 읽어도 됩니다.
...
bool CWanServer::Disconnect(uint64_t sessionID)
{
    //여기 나중에 바꿔야됨
...
bool CWanServer::SendPacket(uint64_t sessionID, PacketBuffer* packet)
{
    uint16_t index = GetSessionIndex(sessionID);
    if (index >= m_sessions.size() || m_sessions[index] == nullptr ||
        m_sessions[index]->sessionID != sessionID)
...
bool CWanServer::SendPacket(SESSION* session, PacketBuffer* packet)
{
    // 필요에 따라 구현 (현재 단순히 true 반환)
    return true;
}

void CWanServer::SendAllPacket()
{
    //일괄 send를 위한 postQueue
    PostQueuedCompletionStatus(m_iocpHandle, 0, (ULONG_PTR)(-1), NULL);
}

void CWanServer::PostSendAll()
{
    //printf("PostSendAll\n");
...
bool CWanServer::PostRecv(SESSION* session)
{
    if (session->stopIO == 1)
        return true;
...
bool CWanServer::PostSend(SESSION* session)
{ 

    if (session->stopIO == 1)
        return true;

    if (session->sendState.parts.Count == 0)
        return true;

    if (session->sendState.parts.Count > 500)
    {
...
void CWanServer::DisconnectSession(SESSION* session)
{
    InterlockedExchange(&session->stopIO, 1);
      ...
      /*  if (session == nullptr && overlapped == nullptr)
        ...
        if (overlapped == &session->recvOverlapped)
        {
            ...
            while (true)
            {
                ...
                {
                    server->DisconnectSession(session);
                    InterlockedIncrement(&server->disconnectLen);
                    break;
                }
                ...
                //*CreateBuffer << session->sessionID;
                //CreateBuffer->EnqueueData((char*)&conHeader, sizeof(conHeader));

                //OnRecv
                server->m_recvTps[TPSIndex]++;
                server->OnRecv(session->sessionID, packet);
                packet->Release();
              /*  PacketBuffer* packet2 = PacketBuffer::Alloc();
                *packet2 << session->sessionID;
                packet2->EnqueueData(packet->GetReadPtr(), packet->GetDataSize());
                packet->Release();
                

                packet2->AddRef();
                InterlockedIncrement(&jobQueueSize);
                jobQueue.Enqueue(packet2);
                packet2->Release();*/
            }
...
//////////////////////////////////////////////////////////////////////////
unsigned __stdcall CWanServer::AcceptThread(LPVOID param)
{
    CWanServer* server = reinterpret_cast<CWanServer*>(param);
    ...
    while (true)
    {
       ...
       /* PacketBuffer* CreateBuffer = PacketBuffer::Alloc();
        ContentHeader conHeader;
        conHeader.len = 0;
...
//////////////////////////////////////////////////////////////////////////
// Monitor Thread: monitor 이벤트가 신호되면 종료
//////////////////////////////////////////////////////////////////////////
//unsigned __stdcall CWanServer::MonitorThread(LPVOID param)
//{
//    CWanServer* server = reinterpret_cast<CWanServer*>(param);
//    while (true)
//    {
//        DWORD dw = WaitForSingleObject(server->m_monitorEvent, 1000);
//        if (dw == WAIT_OBJECT_0)
//            break;
//
...
//        //("Disconnect  protocol : %lu | checksum : %lu | heartBeat : %lu | packetType : %lu\n", server->disconnectCntProtocol, server->disconnectCntCheckSum, server->disconnectHeartBeat, server->disconnectPacketType);    
//        printf("Disconnect  protocol : %lu | checksum : %lu | len : %lu | sameID : %lu | packetType : %lu\n", 
//            server->disconnectCntProtocol, server->disconnectCntCheckSum, server->disconnectLen ,sameIDDisconnectCnt, server->disconnectPacketType);
//
//        printf("wrong IP : %lu\n", server->wrongIP);
//        if (server->wrongIP != 0)
//        {
...
//            {
//                printf("wrong serverIP : %s\n", server->ips[i].c_str());
//            }
//        }
//
//        //printf("CharacterCnt : %d / SectorCnt : %d / stackSize : %d\n", GetCharacterCnt(), CheckTotalSector(), stackSize);
//       // printf("frame : %d ,jobQueueTPS : %lu\n", frameCnt, jobQueueTPS);
//        //printf("frame : %d , jobQueueSize : %lu, successSize : %lu, jobQueueTPS : %lu\n", frameCnt, jobQueueSize, successSize, jobQueueTPS);
//       // frameCnt = 0;
//       // jobQueueTPS = 0;
//
...
// Start / Stop
//////////////////////////////////////////////////////////////////////////
bool CWanServer::Start(const char* ip, int port, int workerThreadCount, int conCurrentThreadCount, int maxConnections, int maxPlayers, bool useNagle)
{
