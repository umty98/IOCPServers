...
PacketBuffer& PacketBuffer::operator<<(INT64 value)
{
	if (p_WritePos + sizeof(INT64) > p_BufferSize)
	{
		if (!ResizeBuffer(p_BufferSize * 2))
		{
			p_Error = true;
	...
	*(INT64*)(p_Buffer + p_WritePos) = value;
	p_WritePos += sizeof(INT64);
	p_DataSize = p_WritePos;
	return *this;
}
...
PacketBuffer& PacketBuffer::operator>>(INT64& value)
{
	if (p_ReadPos + sizeof(INT64) > p_DataSize)
	{
		value = 0;
		p_Error = true;
		return *this;
	}
	value = *(INT64*)(p_Buffer + p_ReadPos);
	p_ReadPos += sizeof(INT64);
	return *this;
}
...
void PacketBuffer::SetCheckSum(PacketBuffer* buffer)
{
	...
	char* ptr = start + sizeof(CWanServer::NetWorkHeader);
...
{
	int dataSize = buffer->GetDataSize();
	char* start = buffer->GetBufferPtr() + sizeof(CWanServer::NetWorkHeader);
	char* ptr = start;
	char* end = start + dataSize;
	BYTE checkSum = 0;
