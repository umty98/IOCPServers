...
unsigned __stdcall MonitoringThreadProc(void* p)
{
    ...
    while (true)
    {
        ...
        if (now >= nextPrint)
        {
            ...
            printf("acceptTPS : %lu | packetTPS : %lu | loginTPS : %lu | sectorTPS : %lu | chatTPS : %lu\n", server.acceptTPS, packetTPS, loginTPS, sectorTPS, chatTPS);
            printf("sendFrame : %lu\n", sendCnt);
            sendCnt = 0;

            printf("Disconnect  protocol : %lu | checksum : %lu | sameID : %lu | deleteID : %lu\n", server.disconnectCntProtocol, server.disconnectCntCheckSum, sameIDDisconnectCnt, deleteIDErrorCnt);
            printf("contentTypeErrorCnt : %lu | contentErrorCnt : %lu | contentLenErrorCnt : %lu\n", typeErrorCnt, contentErrorCnt, contentLenErrorCnt);
            printf("redisDisconnectCnt : %lu | loginwice : %lu | loginSector : %lu | loginChat : %lu\n", redisDisconnectCnt, loginTwiceDisconnectCnt, loginNotreceiveSectorCnt, loginNotReceiveChatCnt);
            /////PDHMonitoring and CPUusage
            printf("------------PDH--------------\n");

            PacketBuffer* buffer0 = client.CreatePacketBuffer();
            mpMonitorDataUpdate(buffer0, dfMONITOR_DATA_TYPE_CHAT_SERVER_RUN, isClientConnected, nowDB);
            ...
            mpMonitorDataUpdate(buffer1, dfMONITOR_DATA_TYPE_CHAT_SERVER_CPU, ProcessTime.GetTotal(), nowDB);
            ...
            mpMonitorDataUpdate(buffer2, dfMONITOR_DATA_TYPE_CHAT_SERVER_MEM, pdhMonitor.GetProcessUserMemory(), nowDB);
            ...
            mpMonitorDataUpdate(buffer3, dfMONITOR_DATA_TYPE_CHAT_SESSION, cnt, nowDB);
            Lan_SendPacket_Unicast(clientID, buffer3);
            buffer3->Release();

            PacketBuffer* buffer4 = client.CreatePacketBuffer();
            mpMonitorDataUpdate(buffer4, dfMONITOR_DATA_TYPE_CHAT_PLAYER, server.currentPlayerCnt, nowDB);
            Lan_SendPacket_Unicast(clientID, buffer4);
            buffer4->Release();

            PacketBuffer* buffer5 = client.CreatePacketBuffer();
            mpMonitorDataUpdate(buffer5, dfMONITOR_DATA_TYPE_CHAT_UPDATE_TPS, packetTPS, nowDB);
            Lan_SendPacket_Unicast(clientID, buffer5);
            buffer5->Release();

            PacketBuffer* buffer6 = client.CreatePacketBuffer();
            mpMonitorDataUpdate(buffer6, dfMONITOR_DATA_TYPE_CHAT_PACKET_POOL, TLSMemoryPool<PacketBuffer>::GetCount(), nowDB);
            ...
            InterlockedExchange(&sectorTPS, 0);
            InterlockedExchange(&chatTPS, 0);
